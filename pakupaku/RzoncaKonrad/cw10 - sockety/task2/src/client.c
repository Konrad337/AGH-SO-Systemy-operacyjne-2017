...
void *run_client_name(void *args) {
    ...
    printf(KBLU "Connecting to socket with SOCK_DGRAM" RESET "\n");
    ...
    if(bind(client_socket_fd, (struct sockaddr *)&server_socket, sizeof(sa_family_t)) == -1) {
    ...
    *sa_len = sizeof(sa);
    ...
    buf -> msg_type = 0;
    buf -> int_msg1 = client_socket_fd;
    strcpy(buf -> string_msg, init_info -> client_name);
    printf("Sending client name: %s\n", buf -> string_msg);
    sendto(client_socket_fd, buf, sizeof(struct message), 0, (struct sockaddr *)&server_socket, sizeof(server_socket));
    while(1) {
        recv(client_socket_fd, buf, sizeof(struct message), 0);
